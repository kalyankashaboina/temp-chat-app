...
import type {
  Message, FileAttachment, Conversation, User,
  QueuedItem, Language, CallRecord, MessageReaction, ReplyTo,
} from '@/features/chat/types';
import { validateFile, getFileType, mockLoadOlderMessages, type PaginationCursor } from '@/features/chat/services/messageService';
...
export const fetchUsers = createAsyncThunk(
  'chat/fetchUsers',
  async (q?: string) => chatApi.getUsers(q),
);
  ...
  async (
    ...
    if (!state.isOnline) {
      dispatch(enqueueItem({
        id: `q-${tempId}`, type: 'message', data: optimistic,
        messageId: convId, retryCount: 0, maxRetries: 3,
    ...
    socketClient.sendMessage({ conversationId: convId, content, tempId, replyTo: state.replyingTo });
    ...
    for (const att of attachments.filter(a => a.uploadStatus === 'pending')) {
      ...
      if (!rawFile) continue;
      try {
        dispatch(updateAttachmentProgress({ conversationId: convId, messageId: tempId, fileId: att.id, progress: 0 }));
        const uploaded = await chatApi.uploadFile(rawFile, (pct) => {
          dispatch(updateAttachmentProgress({ conversationId: convId, messageId: tempId, fileId: att.id, progress: pct }));
        });
        dispatch(updateAttachmentStatus({ conversationId: convId, messageId: tempId, fileId: att.id, status: 'sent', progress: 100 }));
        // TODO: include uploaded URL in message via socket
      } catch {
        dispatch(updateAttachmentStatus({ conversationId: convId, messageId: tempId, fileId: att.id, status: 'failed', progress: 0 }));
      }
    }
  },
);
...
export const startNewChatAsync = createAsyncThunk(
  'chat/startNewChatAsync',
  async (user: User) => {
...
export const createGroupAsync = createAsyncThunk(
  ...
  async ({ name, members }: { name: string; members: User[] }) => {
    return chatApi.createGroup(name, members.map(m => m.id));
  },
);
...
export const retryFileUploadAsync = createAsyncThunk(
  ...
  async (_: { messageId: string; fileId: string }) => { /* handled by component */ },
);
...
const chatSlice = createSlice({
  ...
  initialState,
  reducers: {
    setCurrentUserId: (s, a: PayloadAction<string>) => { s.currentUserId = a.payload; },
    setOnline: (s, a: PayloadAction<boolean>) => { s.isOnline = a.payload; },
    toggleOnline: (s) => { s.isOnline = !s.isOnline; },
    setTyping: (s, a: PayloadAction<boolean>) => { s.isTyping = a.payload; },
    setTypingUsers: (s, a: PayloadAction<string[]>) => { s.typingUsers = a.payload; },
    addTypingUser: (s, a: PayloadAction<string>) => { if (!s.typingUsers.includes(a.payload)) s.typingUsers.push(a.payload); },
    ...
    setShowConversationList: (s, a: PayloadAction<boolean>) => { s.showConversationList = a.payload; },
    setSearchQuery: (s, a: PayloadAction<string>) => { s.searchQuery = a.payload; },
    setReplyingTo: (s, a: PayloadAction<ReplyTo | null>) => { s.replyingTo = a.payload; },
    setLanguage: (s, a: PayloadAction<Language>) => { s.language = a.payload; },
    ...
    addIncomingMessage: (s, a: PayloadAction<{ conversationId: string; message: Message }>) => {
      ...
      if (s.activeConversationId !== conversationId) {
        const conv = s.conversations.find(c => c.id === conversationId);
        if (conv) { conv.unreadCount += 1; conv.lastMessage = message; }
      }
    },

    // When server confirms a message sent (tempId → real id)
    confirmMessage: (s, a: PayloadAction<{ conversationId: string; tempId: string; realId: string; createdAt: string }>) => {
      ...
      if (msg) { msg.id = realId; msg.status = 'sent'; msg.timestamp = new Date(createdAt); }
    },

    updateMessageStatus: (s, a: PayloadAction<{ conversationId: string; messageId: string; status: Message['status'] }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (msgs) { const m = msgs.find(m => m.id === a.payload.messageId); if (m) m.status = a.payload.status; }
    },
    resetMessageForRetry: (s, a: PayloadAction<{ conversationId: string; messageId: string }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (msgs) { const m = msgs.find(m => m.id === a.payload.messageId); if (m) { m.status = 'pending'; m.attachments = m.attachments.map(at => at.uploadStatus === 'failed' ? { ...at, uploadStatus: 'pending' as const, uploadProgress: 0 } : at); } }
    },

    editMessage: (s, a: PayloadAction<{ messageId: string; newContent: string }>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload.messageId);
      if (m) { m.content = a.payload.newContent; m.isEdited = true; m.editedAt = new Date(); }
    ...
    deleteMessage: (s, a: PayloadAction<string>) => {
      ...
      if (m) { m.isDeleted = true; m.content = ''; }
      socketClient.deleteMessage(a.payload);
    },

    applyRemoteDelete: (s, a: PayloadAction<{ conversationId: string; messageId: string }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (msgs) { const m = msgs.find(m => m.id === a.payload.messageId); if (m) { m.isDeleted = true; m.content = ''; } }
    },

    applyRemoteEdit: (s, a: PayloadAction<{ conversationId: string; messageId: string; content: string; editedAt: string }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (msgs) { const m = msgs.find(m => m.id === a.payload.messageId); if (m) { m.content = a.payload.content; m.isEdited = true; m.editedAt = new Date(a.payload.editedAt); } }
    },

    addReaction: (s, a: PayloadAction<{ messageId: string; emoji: string }>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload.messageId);
      if (m) { if (!m.reactions) m.reactions = []; m.reactions.push({ emoji: a.payload.emoji, userId: s.currentUserId, userName: 'You', timestamp: new Date() }); }
    ...
    removeReaction: (s, a: PayloadAction<{ messageId: string; emoji: string }>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload.messageId);
      if (m?.reactions) m.reactions = m.reactions.filter(r => !(r.emoji === a.payload.emoji && r.userId === s.currentUserId));
      socketClient.unreact(a.payload.messageId, a.payload.emoji, convId);
    },
    applyRemoteReaction: (s, a: PayloadAction<{ conversationId: string; messageId: string; emoji: string; userId: string; username: string; added: boolean }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (!msgs) return;
      ...
      if (a.payload.added) {
        if (!m.reactions.some(r => r.emoji === a.payload.emoji && r.userId === a.payload.userId))
          m.reactions.push({ emoji: a.payload.emoji, userId: a.payload.userId, userName: a.payload.username });
      } else {
        m.reactions = m.reactions.filter(r => !(r.emoji === a.payload.emoji && r.userId === a.payload.userId));
      }
    },
    ...
    starMessage: (s, a: PayloadAction<string>) => {
      ...
      chatApi.starMessage(a.payload).catch(() => { if (m) m.isStarred = false; });
    },
    unstarMessage: (s, a: PayloadAction<string>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload);
      if (m) m.isStarred = false;
      chatApi.unstarMessage(a.payload).catch(() => { if (m) m.isStarred = true; });
    },
    pinMessage: (s, a: PayloadAction<string>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload);
      if (m) m.isPinned = true;
      chatApi.pinMessage(a.payload).catch(() => { if (m) m.isPinned = false; });
    },
    unpinMessage: (s, a: PayloadAction<string>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload);
      if (m) m.isPinned = false;
      chatApi.unpinMessage(a.payload).catch(() => { if (m) m.isPinned = true; });
    },
    ...
    markViewOnceAsViewed: (s, a: PayloadAction<{ messageId: string; attachmentId: string }>) => {
      const convId = s.activeConversationId; if (!convId) return;
      const m = (s.messagesMap[convId] || []).find(m => m.id === a.payload.messageId);
      if (m) { const att = m.attachments.find(a2 => a2.id === a.payload.attachmentId); if (att) att.isViewed = true; }
    },

    updateAttachmentProgress: (s, a: PayloadAction<{ conversationId: string; messageId: string; fileId: string; progress: number }>) => {
      const { conversationId, messageId, fileId, progress } = a.payload;
      const m = (s.messagesMap[conversationId] || []).find(m => m.id === messageId);
      if (m) { const att = m.attachments.find(a2 => a2.id === fileId); if (att) att.uploadProgress = progress; }
    },
    updateAttachmentStatus: (s, a: PayloadAction<{ conversationId: string; messageId: string; fileId: string; status: Message['status']; progress: number }>) => {
      const { conversationId, messageId, fileId, status, progress } = a.payload;
      const m = (s.messagesMap[conversationId] || []).find(m => m.id === messageId);
      if (m) { const att = m.attachments.find(a2 => a2.id === fileId); if (att) { att.uploadStatus = status; att.uploadProgress = progress; } }
    },

    startNewChat: (s, a: PayloadAction<User>) => {
      const existing = s.conversations.find(c => !c.isGroup && c.user?.id === a.payload.id);
      if (existing) { s.activeConversationId = existing.id; s.showConversationList = false; return; }
      // Optimistic local — async version creates real conv
      const newConv: Conversation = { id: `temp-${a.payload.id}`, user: a.payload, isGroup: false, unreadCount: 0, typingUsers: [] };
    ...
    upsertConversation: (s, a: PayloadAction<Conversation>) => {
      ...
      if (idx >= 0) s.conversations[idx] = a.payload; else s.conversations.unshift(a.payload);
    ...
    createGroup: (s, a: PayloadAction<{ name: string; members: User[] }>) => {
      ...
      const newGroup: Conversation = {
        id: `grp-${Date.now()}`, users: a.payload.members,
        isGroup: true, groupName: a.payload.name, unreadCount: 0, typingUsers: [],
      };
    ...
    toggleVanishMode: (s, a: PayloadAction<{ conversationId: string; enabled: boolean; timer?: number }>) => {
      const conv = s.conversations.find(c => c.id === a.payload.conversationId);
      if (conv) { conv.isVanishMode = a.payload.enabled; conv.vanishTimer = a.payload.timer ?? 60; }
    },
    ...
    deleteConversation: (s, a: PayloadAction<string>) => {
      ...
      if (s.activeConversationId === a.payload) { s.activeConversationId = null; s.showConversationList = true; }
    },
    updateUserPresence: (s, a: PayloadAction<{ userId: string; isOnline: boolean }>) => {
      s.conversations.forEach(c => {
        ...
        c.users?.forEach(u => { if (u.id === a.payload.userId) u.isOnline = a.payload.isOnline; });
      });
    },
    setOnlineUsers: (s, a: PayloadAction<string[]>) => {
      ...
      s.conversations.forEach(c => {
        ...
        c.users?.forEach(u => { u.isOnline = set.has(u.id); });
      });
    },
    ...
    enqueueItem: (s, a: PayloadAction<QueuedItem>) => { s.queue.push(a.payload); },
    dequeueItem: (s, a: PayloadAction<string>) => { s.queue = s.queue.filter(q => q.id !== a.payload); },
    setProcessingQueue: (s, a: PayloadAction<boolean>) => { s.isProcessingQueue = a.payload; },

    removeVanishedMessage: (s, a: PayloadAction<{ conversationId: string; messageId: string }>) => {
      const msgs = s.messagesMap[a.payload.conversationId];
      if (msgs) s.messagesMap[a.payload.conversationId] = msgs.filter(m => m.id !== a.payload.messageId);
    },
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchConversations.rejected, (s) => { s.isLoadingConversations = false; })
      ...
      .addCase(fetchMessages.rejected, (s) => { s.isLoadingMessages = false; })

      .addCase(fetchUsers.fulfilled, (s, a) => { s.allUsers = a.payload; })

      .addCase(loadMoreMessagesAsync.pending,   (s) => { s.isLoadingMore = true; })
      ...
      .addCase(loadMoreMessagesAsync.rejected, (s) => { s.isLoadingMore = false; })

      .addCase(startNewChatAsync.fulfilled, (s, a) => {
        if (!a.payload) return;
        const { conv } = a.payload;
        const idx = s.conversations.findIndex(c => c.id === conv.id || c.id === `temp-${conv.user?.id}`);
...
export const {
  setCurrentUserId, setOnline, toggleOnline, setTyping, setTypingUsers,
  addTypingUser, removeTypingUser, setShowConversationList, setSearchQuery,
  setReplyingTo, setLanguage, setActiveConversation,
  addOwnMessage, addIncomingMessage, confirmMessage, updateMessageStatus,
  resetMessageForRetry, editMessage, deleteMessage, applyRemoteDelete,
  applyRemoteEdit, addReaction, removeReaction, applyRemoteReaction,
  starMessage, unstarMessage, pinMessage, unpinMessage, forwardMessage,
  markViewOnceAsViewed, updateAttachmentProgress, updateAttachmentStatus,
  startNewChat, upsertConversation, createGroup, toggleVanishMode,
  muteConversation, archiveConversation, pinConversation, deleteConversation,
  updateUserPresence, setOnlineUsers, addCallToHistory,
  enqueueItem, dequeueItem, setProcessingQueue, removeVanishedMessage,
} = chatSlice.actions;
