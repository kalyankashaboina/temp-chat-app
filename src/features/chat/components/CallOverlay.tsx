    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      {isVideo ? (
        <div className="relative flex-1 bg-black">
          ...
          {isConnected && callState.localStream && !callState.isVideoOff && (
            <div className="absolute top-4 right-4 h-32 w-24 overflow-hidden rounded-xl border-2 border-border bg-black shadow-lg md:h-48 md:w-36">
              <video
                ref={localVideoRef}
                autoPlay
                playsInline
          ...
          {/* User info overlay */}
          <div className="absolute top-4 left-4 flex items-center gap-3 rounded-xl bg-black/50 px-4 py-2 backdrop-blur-sm">
        ...
        <div className="relative flex flex-1 flex-col items-center justify-center bg-gradient-to-br from-primary/20 via-background to-secondary/20 p-8">
          ...
          <div className="mt-2 flex items-center gap-2 text-muted-foreground">
            ...
            {isConnected && (
              <span className="text-status-online font-medium text-lg">
                {formatDuration(callState.duration)}
              </span>
            )}
          </div>
      ...
      {/* Controls */}
      <div className="flex items-center justify-center gap-4 bg-card/80 backdrop-blur-sm p-6 md:p-8">
        {/* Mute button */}
        <Button
          variant="secondary"
          size="lg"
