...
function mapMessage(m: ApiMessage, currentUserId: string): Message {
  ...
  return {
    ...
    attachments: (m.attachments ?? []).map((a) => ({
      ...
      name: a.name,
      size: a.size,
      type: a.type as import('@/features/chat/types').FileAttachment['type'],
      mimeType: a.mimeType,
      url: a.url,
      uploadProgress: 100,
      uploadStatus: 'sent' as const,
    })),
...
export const chatApi = {
  ...
  async createDirect(targetUserId: string): Promise<Conversation> {
    const res = await api.post<{ data: ApiConversation }>('/conversations/direct', {
      targetUserId,
    });
  ...
  async createGroup(name: string, memberIds: string[]): Promise<Conversation> {
    const res = await api.post<{ data: ApiConversation }>('/conversations/group', {
      name,
      memberIds,
    });
    return mapConversation(res.data);
  ...
  async getMessages(
    ...
    return {
      messages: res.messages.map((m) => mapMessage(m, currentUserId)),
      nextCursor: res.nextCursor,
      hasMore: res.hasMore,
    };
  },
  ...
  ): Promise<{ url: string; name: string; type: string; mimeType: string; size: number }> {
